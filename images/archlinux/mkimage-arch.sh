...
ROOTFS=$(mktemp -d ${TMPDIR:-/var/tmp}/rootfs-archlinux-XXXXXXXXX)
chmod 755 $ROOTFS
...
case "$arch" in
	armv*)
		...
		PACMAN_MIRRORLIST='Server = http://mirrors.ustc.edu.cn/archlinuxarm/$arch/$repo'
	...
	*)
		...
		PACMAN_MIRRORLIST='Server = https://mirrors.ustc.edu.cn/archlinux/$repo/os/$arch'
		PACMAN_EXTRA_PKGS=''
		EXPECT_TIMEOUT=60
...
arch-chroot $ROOTFS /bin/sh -c "ln -s /usr/share/zoneinfo/PRC /etc/localtime"
