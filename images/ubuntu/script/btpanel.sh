	...
	if [ $(id -u) != "0" ];then
		echo -e "\033[31mError: Please run the script with root privileges on Ubuntu, for example: sudo bash $0\033[0m";
		exit;
	fi

	if [[ `cat /etc/issue | grep buntu` == '' ]]; then
	...
	apt-get install -y --no-install-recommends wget curl zip unzip vim git
	apt-get install -y --no-install-recommends python python-pip python-dev python-imaging python-pil openssl libssl-dev gcc libxml2 libxml2-dev libxslt zlib1g zlib1g-dev libjpeg-dev libpng-dev lsof libpcre3 libpcre3-dev cron
	set_pip	

	# apt-get install -y --no-install-recommends ntp ntpdate
	...
	echo "=>BT Panel will be install to the $setup_path directory now"
