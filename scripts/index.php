...
<script language="JavaScript" type="text/javascript" src="https://cdn.staticfile.org/jquery/1.10.0/jquery.min.js"></script>
</head>
...
<table>
  ...
  <tr>
    <td>服务器域名/IP地址</td>
    <td colspan="3"> <?php echo $_SERVER['SERVER_NAME'];?>(<?php if('/'==DIRECTORY_SEPARATOR){echo $_SERVER['SERVER_ADDR'];}else{echo @gethostbyname($_SERVER['SERVER_NAME']);} ?>)&nbsp;&nbsp;你的 IP 地址是：<?php echo @$_SERVER['REMOTE_ADDR'];?></td>
  </tr>
...
<table>
  ...
  <tr>
    <td>CPU使用状况</td>
    <td colspan="5"><?php if('/'==DIRECTORY_SEPARATOR){echo $cpu_show." | <a href='".$phpSelf."?act=cpu_percentage' target='_blank' class='static'>";}else{echo "暂时只支持Linux系统";}?>
    </td>
  </tr>
  <tr>
    <td>硬盘使用状况</td>
...
<table>
  ...
  <tr>
    ...
    <td width="20%"><?php if(isset(opcache_get_status()['jit']['enabled']) && opcache_get_status()['jit']['enabled'] ==true){echo is_enable(true);}else{echo is_enable(false);} ?></td>
  ...
  <tr>
    <td>GD 库</td><td><?php echo is_ext('gd',gd_info()['GD Version']);?></td>
    <td>压缩文件(Zlib)</td><td><?php echo isfun("gzclose");?></td>
  </tr>
...
<tr>
    <td>Zend OPcache</td><td><?php echo is_ext('Zend OPcache');?>
    <td>Redis</td><td><?php echo is_ext('Redis')?></td>
</tr>
<tr>
    <td>XCache</td><td><?php echo is_ext('XCache')?></td>
    <td>Swoole</td><td><?php echo is_ext('swoole');?>
  </tr>
  <tr>
    <td>mongodb</td><td><?php echo is_ext('mongodb')?></td>
    <td>Memcached</td><td><?php echo is_ext('memcached');?>
  </tr>
  <tr>
    <td>amqp</td><td><?php echo is_ext('amqp')?></td>
    <td>Kafaka</td><td><?php echo is_ext('rdkafka');?>
  </tr>
  <tr>
    <td>FFI</td><td><?php echo is_ext('FFI')?></td>
    <td>xhprof</td><td><?php echo is_ext('xhprof');?>
  </tr>
  <tr>
    <td>Yaf</td><td><?php echo is_ext('yaf')?></td>
    <td>Yac</td><td><?php echo is_ext('yac');?>
  </tr>
  <tr>
    <td>XDebug</td><td><?php echo is_ext('xdebug')?></td>
    <td>Phalcon</td><td><?php echo is_ext('phalcon');?>
  </tr>
  <tr>
    <td>Source Guardian</td><td><?php echo is_ext('sourceguardian')?></td>
    <td>APCu</td><td><?php echo is_ext('apcu');?>
  </tr>
</table>

<a name="w_db" id="w_db" style="position:relative;top:-60px;"></a>
<table>
  <tr><th colspan="4"><i class="fa fa-database"></i> 数据库</th></tr>
